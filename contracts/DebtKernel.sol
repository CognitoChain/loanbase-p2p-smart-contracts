...
contract DebtKernel is Pausable {
        ...
        returns (bool _orderIsConsensual)
    {
        ...
        if (msg.sender != debtOrder.issuance.debtor) {
            if (!isValidSignature(
             ...
             emit   LogError(uint8(Errors.ORDER_INVALID_NON_CONSENSUAL), debtOrder.debtOrderHash);
                return false;
            }
        }
        ...
        returns (bool _orderIsValid)
    {
        ...
        if (totalFees != debtOrder.relayerFee.add(debtOrder.underwriterFee)) {
          emit  LogError(uint8(Errors.ORDER_INVALID_INSUFFICIENT_OR_EXCESSIVE_FEES), debtOrder.debtOrderHash);
            return false;
        }

        // Invariant: debtor is given enough principal to cover at least debtorFees
        if (debtOrder.principalAmount < debtOrder.debtorFee) {
          emit LogError(uint8(Errors.ORDER_INVALID_INSUFFICIENT_PRINCIPAL), debtOrder.debtOrderHash);
            return false;
        }
        ...
        if ((debtOrder.issuance.underwriter == address(0) && debtOrder.underwriterFee > 0) ||
           ...
           emit LogError(uint8(Errors.ORDER_INVALID_UNSPECIFIED_FEE_RECIPIENT), debtOrder.debtOrderHash);
            return false;
        }
        ...
        if (debtOrder.expirationTimestampInSec < block.timestamp) {
          emit  LogError(uint8(Errors.ORDER_EXPIRED), debtOrder.debtOrderHash);
            return false;
        }
        ...
        if (issuanceCancelled[debtOrder.issuance.agreementId]) {
            emit LogError(uint8(Errors.ISSUANCE_CANCELLED), debtOrder.debtOrderHash);
            return false;
        }

        // Invariant: debt order itself must not have been cancelled
        if (debtOrderCancelled[debtOrder.debtOrderHash]) {
            emit LogError(uint8(Errors.ORDER_CANCELLED), debtOrder.debtOrderHash);
            return false;
        }
        ...
        returns (bool _isBalanceAndAllowanceSufficient)
    {
        ...
        if (getBalance(debtOrder.principalToken, creditor) < totalCreditorPayment ||
            ...
            emit LogError(uint8(Errors.CREDITOR_BALANCE_OR_ALLOWANCE_INSUFFICIENT), debtOrder.debtOrderHash);
            return false;
        }
