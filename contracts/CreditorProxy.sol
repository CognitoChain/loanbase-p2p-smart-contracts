...
contract CreditorProxy is Pausable {
        ...
        returns (bytes32 _agreementId)
    {
        ...
        if (debtOfferFilled[creditorCommitmentHash]) {
            emit LogError(uint8(Errors.DEBT_OFFER_ALREADY_FILLED), creditor, creditorCommitmentHash);
            return NULL_ISSUANCE_HASH;
        }

        if (debtOfferCancelled[creditorCommitmentHash]) {
            emit LogError(uint8(Errors.DEBT_OFFER_CANCELLED), creditor, creditorCommitmentHash);
            return NULL_ISSUANCE_HASH;
        }
       ...
       emit LogDebtOfferFilled(creditor, creditorCommitmentHash, agreementId);
        ...
        whenNotPaused
    {
        ...
        emit LogDebtOfferCancelled(commitmentAddresses[0], creditorCommitmentHash);
    }
    ...
    {
        return keccak256(abi.encodePacked(commitmentAddresses[0], // creditor
